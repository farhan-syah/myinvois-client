...
export interface UBLJsonSelfBilledCreditNoteV1_1_Content {
  ...
  DocumentCurrencyCode: UBLJsonValue<string>[]; // Mandatory [1-1], Using Array as per UBLJsonValue structure pattern
  /** Optional Tax Currency Code. Maps to UBL: / ubl:Invoice / cbc:TaxCurrencyCode */
  TaxCurrencyCode?: UBLJsonValue<string>[]; // Optional [0-1]
  ...
  AccountingSupplierParty: UBLJsonAccountingSupplierParty[]; // Mandatory [1-1], Array as per UBLJson structure pattern
  /** Structure representing the buyer information (Buyer issuing the note). Maps to UBL: / ubl:Invoice / cac:AccountingCustomerParty */
  AccountingCustomerParty: UBLJsonAccountingCustomerParty[]; // Mandatory [1-1], Array as per UBLJson structure pattern
  /** Optional Billing period information. Maps to UBL: / ubl:Invoice / cac:InvoicePeriod */
  InvoicePeriod?: UBLJsonInvoicePeriod[]; // Optional [0-1], Array as per UBLJson structure pattern
  ...
  BillingReference?: UBLJsonBillingReference[]; // Mandatory where applicable [0-*], Array as per UBLJson structure pattern
  ...
  AdditionalDocumentReference?: UBLJsonAdditionalDocumentReference[]; // Optional [0-*], Array as per UBLJson structure pattern
  /** Optional Delivery information. Maps to UBL: / ubl:Invoice / cac:Delivery */
  Delivery?: UBLJsonDelivery[]; // Optional [0-1], Array as per UBLJson structure pattern
  /** Optional Payment means information. Maps to UBL: / ubl:Invoice / cac:PaymentMeans */
  PaymentMeans?: UBLJsonPaymentMeans[]; // Optional [0-1], Array as per UBLJson structure pattern
  /** Optional Payment terms. Maps to UBL: / ubl:Invoice / cac:PaymentTerms */
  PaymentTerms?: UBLJsonPaymentTerms[]; // Optional [0-1], Array as per UBLJson structure pattern
  /** Optional Prepaid payment information. Maps to UBL: / ubl:Invoice / cac:PrepaidPayment */
  PrepaidPayment?: UBLJsonPrepaidPayment[]; // Optional [0-1], Array as per UBLJson structure pattern
  /** Optional Document level allowances or charges (discounts/fees). Maps to UBL: / ubl:Invoice / cac:AllowanceCharge */
  AllowanceCharge?: UBLJsonFreightAllowanceCharge[]; // Optional [0-*], Array as per UBLJson structure pattern
  /** Optional Currency exchange rate information. Maps to UBL: / ubl:Invoice / cac:TaxExchangeRate */
  TaxExchangeRate?: UBLJsonTaxExchangeRate[]; // Mandatory where applicable [0-1], Array as per UBLJson structure pattern
  /** Total tax information for the invoice. Maps to UBL: / ubl:Invoice / cac:TaxTotal */
  TaxTotal: UBLJsonTaxTotal[]; // Mandatory [1-1], Array as per UBLJson structure pattern
  /** Legal monetary total summary for the invoice. Maps to UBL: / ubl:Invoice / cac:LegalMonetaryTotal */
  LegalMonetaryTotal: UBLJsonLegalMonetaryTotal[]; // Mandatory [1-1], Array as per UBLJson structure pattern
  /** Invoice line items. Maps to UBL: / ubl:Invoice / cac:InvoiceLine */
  InvoiceLine: UBLJsonInvoiceLine[]; // Mandatory [1-*], Array as per UBLJson structure pattern
  ...
  Signature: UBLJsonSignature[]; // Mandatory [1-1] for v1.1
...
export interface UBLJsonSelfBilledCreditNoteDocumentV1_1 {
  ...
  Invoice: UBLJsonSelfBilledCreditNoteV1_1_Content[]; // [1-1] based on root document structure samples
}
...
export interface UBLJsonSelfBilledCreditNoteDocumentV1_0 {
  ...
  Invoice: UBLJsonSelfBilledCreditNoteV1_0_Content[]; // [1-1] based on root document structure samples
}
